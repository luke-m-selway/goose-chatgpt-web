...
const en = {
  ...
  mcpStepThreeBody: "Before creating the connector, enable Developer Mode in ChatGPT Settings. In ChatGPT Plugins, create a new connector, choose Tunnel, select the tunnel you created, set Authentication to None, and name it exactly Goose Native. Open Permissions and choose Allow all actions; Allow low-risk actions blocks command and tool calls before they reach this runtime. The outer harness (Goose or Codex) still enforces its own sandbox and approvals. Then verify the runtime.",
  openConnectors: "Open ChatGPT Plugins",
  connectorMigrationNotice: "Upgrading from Codex Native? Leave the old connector untouched and create Goose Native as a new connector with Allow all actions. Do not rename or refresh Codex Native. Verify runtime rejects a legacy-only setup.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  mcpStepThreeBody: "创建连接器前，先在 ChatGPT 设置中启用开发者模式。然后在 ChatGPT Plugins 中新建连接器，选择 Tunnel，选中刚创建的 Tunnel，将身份验证设为无，并将名称设为 Goose Native。打开权限并选择「允许所有操作」；「允许低风险操作」会在命令和工具调用到达本地运行时之前将其拦截。外层 harness（Goose 或 Codex）仍会执行自己的沙箱与审批规则。最后验证运行时。",
  openConnectors: "打开 ChatGPT Plugins",
  connectorMigrationNotice: "从 Codex Native 升级？请保留旧连接器不变，并新建名为 Goose Native 的连接器，权限选择「允许所有操作」。不要重命名或刷新 Codex Native。仅有旧连接器时，运行时验证会明确失败。",
  connectorName: "连接器名称",
  verifyRuntime: "验证运行时",
  activityTitle: "运行时活动",
