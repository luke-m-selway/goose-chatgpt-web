...
- **Launcher heartbeat/orphan-surface reaper:** keep upstream's newer launcher-lifecycle cleanup as a separate parity/reliability follow-up. It is intentionally deferred from PR #20 because the observed long-running helper case is healthy and the change is wider than the focused compaction/browser-control fixes.
