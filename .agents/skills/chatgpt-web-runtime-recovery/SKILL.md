...
1. **Inspect.** Read `/healthz` and preserve the projected state.
   - If turn counts are already `0/0`, run `codex-chatgpt-web doctor --json`, then read `/healthz` again. If both reads are `0/0` and doctor reports `ok=true`, return **PASS** without cancellation.
...
3. **Verify state.** Read `/healthz` immediately after cancellation.
   ...
   - Once `0/0` is observed, run `codex-chatgpt-web doctor --json`, then read `/healthz` once more. PASS only if doctor reports `ok=true` and that final read is still `0/0`.
...
4. **Handle caller re-admission, bounded.** `cancel-turns` opens the retry circuit for currently retained lineages and clears the current browser-turn registry, but it does not drain the daemon or take scheduling authority away from Goose. A live caller can therefore admit a distinct new operation after a successful cancellation.
   ...
   - After each follow-up, repeat the same `/healthz -> doctor -> /healthz` verification when `0/0` is reached.
...
- **PASS** — authoritative turn counts are `0/0` before and after the supported runtime health check, and `codex-chatgpt-web doctor --json` reports `ok=true`.
- **FAIL** — the supported cancellation command fails, `/healthz` is unavailable/malformed, or existing infrastructure remains unhealthy after clean `0/0` turn state.
...
Return the preflight and final projected `/healthz` fields, every `cancelledBrowserTurns` result, whether re-admission occurred, `doctor --json` health verdict, the already-known causal error/trace identifiers, and whether any process/service was restarted. Do not include secrets.
